...
function computeNextDirection(sn) {
    ...
    // An enemy head is occupied, so A* cannot use it as a goal. Find the
    // shortest safe path to one of the open cells beside the enemy instead.
    function tryPathNextTo(gx, gy, margin) {
        let best = null;
        let bestLength = Infinity;
        for (const [dx, dy] of DIRS) {
            const nx = gx + dx, ny = gy + dy;
            if (!inBounds(nx, ny) || occupied[ny * cols + nx]) continue;
            const path = getPathTo(nx, ny);
            if (!path || path.length < 2 || path.length >= bestLength) continue;
            const next = path[1];
            const need = sn.body.length + margin;
            if (safeSpace(next.x, next.y, need) < need) continue;
            best = { x: next.x - head.x, y: next.y - head.y };
    ...
    if (sn.personality === 'aggressive' && state.snakes.length > 1) {
        ...
        if (target && shouldKill) {
            ...
            // Strategy 2: Direct chase — move to a free cell beside the target.
            // The target head itself is occupied and cannot be an A* goal.
            const headDist = Math.abs(head.x - th.x) + Math.abs(head.y - th.y);
            if (forcedRetaliation || headDist <= AGGRESSIVE_KILL_RANGE + 2) {
                const dir = tryPathNextTo(th.x, th.y, safetyMargin);
                if (dir) return dir;
            }
        }
    }
